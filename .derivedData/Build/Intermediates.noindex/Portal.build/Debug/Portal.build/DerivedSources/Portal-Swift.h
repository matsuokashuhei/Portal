...
#ifndef PORTAL_SWIFT_H
#define PORTAL_SWIFT_H
...
# undef any
# pragma clang attribute push(__attribute__((external_source_symbol(language="Swift", defined_in="Portal",generated_declaration))), apply_to=any(function,enum,objc_interface,objc_category,objc_protocol))
...
@class NSMenu;
SWIFT_CLASS("_TtC6Portal11AppDelegate")
...
SWIFT_CLASS("_TtC6Portal17HintOverlayWindow")
@interface HintOverlayWindow : NSWindow
...
/// SwiftUI’s <code>Settings</code> scene does not natively support ESC key dismissal.
/// By using a custom NSWindow with <code>cancelOperation(_:)</code> override, we can
/// reliably handle ESC key regardless of which control has focus.
SWIFT_CLASS("_TtC6Portal14SettingsWindow")
@interface SettingsWindow : NSWindow
/// Handles the Escape key press to close the window.
/// <code>cancelOperation(_:)</code> is the standard NSResponder method for handling ESC key.
/// Unlike <code>keyDown(with:)</code>, it works reliably even when SwiftUI controls
