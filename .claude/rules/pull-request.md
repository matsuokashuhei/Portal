...
PRを作成する前に、`.claude/rules/code-quality.md` のチェックリストを確認する。
...
1. 指摘内容を分析し、ルール化すべきか判断する
2. 既存の適切なルールファイルがあればそこに追加する
3. なければ新規ルールファイルを `.claude/rules/` に作成する

**既存ルールファイルの対応表:**

| 指摘の種類 | 追加先 |
|-----------|--------|
| 命名規則、コード品質 | `code-quality.md` |
| 設計原則、アーキテクチャ | `solid-principles.md` |
| ドキュメント作成 | `documentation.md` |
| Issue/PR運用 | `pull-request.md`, `issue-driven-development.md` |
| 非同期処理、タイミング | `deterministic-solutions.md` |
| 全体最適化 | `global-optimization.md` |

**新規ルールファイル作成時:**

```bash
# ファイル名は kebab-case で作成
touch .claude/rules/<ルール名>.md
```
